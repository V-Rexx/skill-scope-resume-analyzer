...
function App() {
  ...
  const extractPDFText = async (file) => {
    ...
    const texts = await Promise.all(
      Array.from({length: pdf.numPages},
        (_, i) => pdf.getPage(i+1).then((page) => page.getTextContent().then((tc) => 
  ...
  const analyzeResume = async (text) => {
    ...
    const response = await window.puter.ai.chat(
      [
        {role:"system", content: "You are an exper resume reviewer..."},
    ...
    <div className='min-h-screen bg-main-gradient p-4 sm:p-6 lg:p-8 flex items-center justify-center'>
      <div className='max-w-5xl mx-auto w-full'>
        <div className='text-center mb-6'>
          <h1 className='text-5xl sm:text-6xl lg:text-7xl font-light bg-gradient-to-r
          from-cyan-300 via-teal-200 to-sky-300 bg-clip-text text-transparent mb-2'>
            Skill Scope 
          </h1>
          <h2 className='text-xl sm:text-6xl lg:text-7xl font-light bg-gradient-to-r
          from-cyan-300 via-teal-200 to-sky-300 bg-clip-text text-transparent mb-2'>AI Resume Analyzer</h2>
        ...
        {analysis && uploadedFile && (
            ...
            <div className='file-info-card'>
              ...
              sm:items-center gap-4'>
                <div className='flex items-center gap-4'>
                  <div className='icon-container-xl bg-gradient-to-br from-blue-500/20
          ...
          <div className='score-card'>
            <div className='text-center mb-6'>
              ...
              <div className='relative'>
                <p className='text-6xl sm:text-8xl font-extrabold text-cyan-400
                drop-show-lg'>{analysis.overallScore || "7"}</p>
              </div>
              <div className={`inline-flex items-center gap-2 mt-3 px-4 py-2
                rounded-full ${parseInt(analysis.overallScore) >= 8
                  ...
                  <span className='text-lg'>
                    ...
                    parseInt(analysis.overallScore) >= 6 ? "⭐️" : "📈"}
                  </span>
                  <span className='font-semibold text-lg'>
                      {parseInt(analysis.overallScore) >= 8 
                      ? "Excellent" : parseInt(analysis.overallScore) >=6 ?
          ...
          <div className='grid sm:grid-cols-2 gap-4'>
            <div className='feature-card-green group'>
              <div className='bg-green-500/20 icon-container-lg mx-auto
              mb-3 group-hover:bg-green-400/30 transition-colors'>
                <span className='text-green-300 text-xl'>✓</span>
              </div>
              <h4 className='text-green-300 text-sm font-semibold uppercase
              tracking-wide mb-3'>Top Strengths</h4>
            ...
            <div className='feature-card-orange group'>
              <div className='bg-orange-500/20 icon-container-lg mx-auto
              mb-3 group-hover:bg-orange-400/30 transition-colors'>
                <span className='text-orange-300 text-xl'>⚡️</span>
              </div>
              <h4 className='text-orange-300 text-sm font-semibold uppercase
              tracking-wide mb-3'>Main Improvements</h4>
              <div className='space-y-2 text-left'>
          ...
          <div className='section-card group'>
            ...
            <div className='space-y-4'>
                {METRIC_CONFIG.map((cfg, i) => {
                  const value = analysis.performanceMetrics?.[cfg.key] ??
                  cfg.defaultValue;
                  return <div key={i} className='group/item'>
                    <div className='flex items-center justify-between mb-2'>
                      <div className='flex items-center gap-2'>
                        <span className='text-lg'>{cfg.icon}</span>
                        <p className='text-slate-200'>{cfg.label}</p>
                      </div>
                      <span className='text-slate-300 text-lg'>{value}/10</span>
                    </div>
                    <div className='progress-bar-small'>
                      <div className={`h-full rounded-full transition duration-1000 ease-out group-hover/item:shadow-lg bg-linear-to-r ${cfg.colorClass} ${cfg.shadowClass}`} 
                      style={{width: `${(Number(value)/10) * 100}%`}}></div>
                    </div>
                  </div>
                })}
            </div>
          ...
          <div className='section-card group '>
            <div className='flex items-center gap-3 mb-6'>
              <div className='icon-container bg-violet-500/20'><span className='text-lg'>🤖</span></div>
            ...
            <div className='info-box-violet mb-4'> 
                <div className='flex items-start gap-3 mb-3'>
                  <div>
                    <h3 className='text-violet-300'>What is ATS?</h3>
                    <p className='text-slate-200 text-sm leading-relaxed'>
                        <strong className='text-violet-300'>
                          Applicant Tracking System (ATS)
                        </strong>{" "}
                        are software tools used by 80% of employers to automatically screen resumes
                        before human review. These systems scan for keywords, proper formatting, and relevant
                        qualifications to rank candidates. If your resume isn't ATS-friendly, it may never reach
                        a human recruiter.
                    </p>
                  </div>
                </div>
          ...
          <div className='section-card group'>
            <div className='flex items-center gap-3 mb-6'>
              <div className='icon-container bg-blue-500/20'><span className='text-lg'>🔑</span></div>
            ...
            <div className='info-box-blue'>
                <p className='text-slate-300 text-sm leading-relaxed flex items-start gap-2'>
                  ...
                  Consider incorporating these keywords naturally into your resume to improve ATS
                  compatibility and increase your chances of getting noticed by recruiters.
                </p>
            </div>
          </div>
